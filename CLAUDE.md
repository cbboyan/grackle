...
Once all runner and trainer migrations are complete (solverpy runners, GrackleDomain-based trainers, CVC4 removal), this repository will be migrated as a package named **`solverpy-grackle`** into the solverpy monorepo at `~/repos/cbboyan/solverpy`.

Additional planned work before/during migration:
...
3. **Training**: Using ParamILS (or SMAC3) to tune parameters and invent a new configuration
