...
class kNN : public Predictor {
  public:
    kNN(LVecVec deps, LVecVec syms, LVecVec sym_ths, long sym_num,
      double knn_power, long knn_begin, double knn_distance);
...
: Predictor(deps, syms, sym_ths, sym_num), tfidf(sym_num),
   knn_power(knn_power), knn_begin(knn_begin), knn_distance(knn_distance)
{
}
...
LDPairVec kNN::predict(const LVec& csyms, long maxth, long no_adv) const {
  ...
  for (const auto& sym : csyms) {
    ...
    for (const auto& th : ths) {
      // Jan: the 6 in the line below needs to be optimized
      if (th < maxth) ans[th].second += pow(weight, knn_power);
    }
  }
  ...
  // Jan: change zero in next line to select starting K
  for (long k = knn_begin; k < maxth && no_recommends < no_adv; ++k) {
    ...
    LVec ds = deps[nn];
    // Jan: the 2.7 below is to be optimized, it is the ratio. 0.5 until 10?
    double ol = knn_distance * o / ds.size();
    for (const auto& d : ds) {
