...
class NaiveBayes : public Predictor {
  public:
    NaiveBayes(LVecVec deps, LVecVec syms, LVecVec sym_ths, long sym_num,
       double nbayes_score, double nbayes_freq, 
       double nbayes_log, double nbayes_missing
    );
...
: Predictor(deps, syms, sym_ths, sym_num), tfidf(sym_num),
   nbayes_score(nbayes_score), nbayes_freq(nbayes_freq),
   nbayes_log(nbayes_log), nbayes_missing(nbayes_missing)
{
...
double NaiveBayes::score(sample_t i, set<feature_t> symh) const {
  ...
  for (const auto sv : sfreqh) {
    ...
    if (symh.erase(sv.first) == 1)
      // Jan
      s += tfidf.get(sv.first) * log (nbayes_freq * sfreqv / n);
    else
      // Jan
      s += tfidf.get(sv.first) * nbayes_log * log (1 + (1 - sfreqv) / n);
  }

  // for all query features that did not appear in features of dependencies
  // of current theorem
  // Jan
  for (const auto f : symh) s -= tfidf.get(f) * nbayes_missing;
